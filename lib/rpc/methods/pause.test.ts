...
describe('aria2.pause', () => {
  it('delegates to downloadManager.pause and returns GID', async () => {
    const registry = new MethodRegistry();
    const pauseMock = vi.fn().mockResolvedValue(undefined);
    const ctx = createMockCtx({
      downloadManager: { pause: pauseMock } as unknown as DownloadManager,
    });
    ...
    expect(pauseMock).toHaveBeenCalledWith('gid-001');
  });
